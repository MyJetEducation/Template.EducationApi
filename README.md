![.NET Core](https://github.com/MyJetEducation/Template.EducationApi/workflows/.NET%20Core/badge.svg)
...
where ${path} is the **full** path to the clonned directory (where the folder .template.config placed) **without trailing slash** (e.g. `D:\Root\Template.EducationApi\template`)

### Create

`
dotnet new myjeteducationtutorialapiservice -n {ServiceName} -o {ServiceDirectory}
`

e.g.

`
dotnet new myjeteducationtutorialapiservice -n SomeService -o SomeServiceFoulder
`
